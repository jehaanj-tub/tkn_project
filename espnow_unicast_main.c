...
//////////////////////////////////////////////////////////////////////////////////////////
bool sender = true; // Set to true if sender, false if receiver                         //
                                                                                        //
// Uncomment address of peer (modify address of peer if needed)                      //                                                                  //
...
static void send_data(example_espnow_send_param_t *send_param, const uint8_t *mac_addr, char *data, int size)
{
    ...
    send_param->len = sizeof(example_espnow_data_t) + size;
...
static void example_espnow_task(void *pvParameter)
{
    ...
    if(sender == true) {
        send_data(send_param, my_peer, "RTS", 4);
    }
        ...
        switch (evt.id) {
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    send_data(send_param, recv_cb->mac_addr, "RTS", 4);
                }
                    ...
                    if(strcmp((char *)payload,"RTS") == 0){
                        ...
                        send_data(send_param, recv_cb->mac_addr, "CTS", 4);
                    }
                    else if(strcmp((char *)payload,"CTS") == 0){
                        example_espnow_send_param_t *send_param = malloc(sizeof(example_espnow_send_param_t));

                        send_data(send_param, recv_cb->mac_addr, "Some data", 10);
                    }
                    else if(strcmp((char *)payload,"Some data") == 0){
                        example_espnow_send_param_t *send_param = malloc(sizeof(example_espnow_send_param_t));

                        send_data(send_param, recv_cb->mac_addr, "ACK", 4);
                    }
